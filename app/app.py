...
const String ssid     = "SSIDPLACEHOLDER";
const String password = "PASSWORDPLACEHOLDER";
...
class Device
{
  public:
    ...
    void sendData() {
      ...
      JSONencoder["token"] = "TOKENPLACEHOLDER";
...
# def get_ssids():
#     try:
#         MyOut = subprocess.run(['netsh', 'wlan', 'show', 'network'],
#                                capture_output=True, text=True)
#         stdout = MyOut.stdout
#         string = re.compile(r"\s+").sub(" ", stdout)
#         temp = re.compile(r"SSID \d+ : (.*?) ")
#         return temp.findall(string)
#     except:
#         return []
...
class MainWindow(sg.Window):
    def __init__(self):
        ...
        self.username = ""
        self.password = ""
        self._layout = [[sg.Text("Username")],
                        [sg.InputText(key="username")],
                        [sg.Text("Password")],
                        [sg.InputText(key="password")],
                        # [sg.Text("Wifi Picker")],
                        # [sg.Combo(
                        #     self.ssids, enable_events=True, key="wifisElem", size=(50, 5))],
                        [sg.Text('SSID')],
                        [sg.InputText(key="ssid")],
                        [sg.Text("Wifi Password")],
                        [sg.InputText(key="pwd")],
                        # [sg.Button("Refresh", key="refreshBtn")],
                        [sg.Button("Create", key="createBtn"), sg.Cancel()]]
        super().__init__("ESP32", self._layout)
        self.run()
    ...
    def run(self):
        while True:
            try:
                ...
                # if event == "wifisElem":
                #     self.onWifiChoiceChanged(values)
                # elif event == "refreshBtn":
                #     self.onRefreshBtnClicked(values)
                if event == "createBtn":
                    self.validateInput(values)
                    token = self.login(values["username"], values["password"])
                    self.create_src(values["ssid"], values["pwd"], token)
                    sg.Popup("コンパイル完了")
    ...
    def validateInput(self, values):
        if max([len(values["ssid"]), len(values["pwd"]), len(values["username"]), len(values["password"])]) == 0:
            raise Exception("未入力の項目があります")

    def onWifiChoiceChanged(self, values):
        self["ssid"].update(values["wifisElem"])

    def onRefreshBtnClicked(self, values):
        pass
        # if len(values["wifisElem"]) <= 0:
        #     self["wifisElem"].update(values=get_ssids(), value="")
        # else:
        #     self["wifisElem"].update(
        #         values=get_ssids(), value=values["wifisElem"])

    def login(self, username, password):
        try:
            url = "https://fc.happysocial.net/api/auth/login"
            loginInfo = {"name": username, "password": password}
            res = requests.post(url, json=loginInfo)
            data = res.json()
            return data["token"]
        except:
            raise Exception("ログインに失敗しました")

    def create_src(self, ssid, pwd, token):
        with open("./src/main.ino", "w") as f:
            f.write(src.replace("SSIDPLACEHOLDER",
                                ssid).replace("PASSWORDPLACEHOLDER", pwd).replace("TOKENPLACEHOLDER", token))
        subprocess.run(['../pypy/bin/platformio', 'run', '-t', 'upload'])
